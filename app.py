    ...
    with st.chat_message("assistant"):
        with st.spinner("Lily 老師正在思考與錄音..."):
            try:
                ...
                # 解析並呈現分離的語音
                parse_and_display_response(response, is_last=True)import streamlit as st
from groq import Groq
from gtts import gTTS
import io
import re
...
st.title("🗣️ 專屬 AI 英文口語教練")
st.caption("UI 完美修正版：輸入區與錄音鈕已固定在最上方，再也不會找不到或卡住了！")
...
with input_container:
    # 建立一組唯一的 key，防止 Streamlit 多輪對話時錄音元件狀態卡死
...
# 3. 顯示歷史對話紀錄（倒序或正序顯示，此處維持正序，但最新回答在最下方）
# 元件置頂後，歷史紀錄往下堆疊，畫面會非常穩定
for idx, msg in enumerate(st.session_state.messages):
    if msg["role"] != "system":
        with st.chat_message(msg["role"]):
            is_last_msg = (idx == len(st.session_state.messages) - 1)
